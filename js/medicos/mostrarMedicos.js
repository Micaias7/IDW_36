...
export const mostrarMedicosEnIndex = () => {
  ...
  medicos.forEach((m) => {
    ...
    card.innerHTML = `
      ...
      <img src="${m.imagen || m.imagenFinal || '../public/doctor.png'}" alt="doctor" class="doc img-fluid mb-2" style="max-width: 160px; border-radius: 10px;">
      <p class="mb-1"><strong>Especialidad:</strong> ${m.especialidad}</p>
      <p class="mb-1"><strong>Matrícula:</strong> ${m.matricula || "-"}</p>
      <p class="mb-1"><strong>Valor consulta:</strong> $${m.valorConsulta?.toFixed(2) || "-"}</p>
      <p class="small text-muted mt-2">${m.descripcion || ""}</p>
      <p class="mt-2"><strong>Obras sociales:</strong> ${
  ...
  medicos.forEach((m, index) => {
    ...
    item.innerHTML = `
      <div class="card mx-auto border-3 border-success rounded-3 shadow text-center p-3" style="max-width: 18rem;">
        <img src="${m.imagen || m.imagenFinal || '../public/doctor.png'}" class="doc card-img-top" alt="doctor" style="height:180px; object-fit:cover; border-radius:10px;">
        <div class="card-body text-center">
          <h5 class="card-title">${m.genero} ${m.nombre} ${m.apellido}</h5>
          <p class="card-text mb-1"><strong>${m.especialidad}</strong></p>
          <p class="card-text small mb-1">Mat. ${m.matricula || "-"}</p>
          <p class="card-text small mb-1">Valor: $${m.valorConsulta?.toFixed(2) || "-"}</p>
          <p class="card-text small">${
            m.obrasSociales && m.obrasSociales.length
              ? m.obrasSociales
                  .map((id) =>
                    id == 1 ? "OSDE" : id == 2 ? "PAMI" : id == 3 ? "IOMA" : "Otra"
                  )
                  .join(", ")
              : ""
          }</p>
...
export const mostrarMedicosEnAlta = () => {
  ...
  medicos.forEach((m) => {
    ...
    tr.innerHTML = `
      ...
      <td>${m.descripcion || "-"}</td>
      <td>
        <img src="${m.imagen}" alt="foto" width="60" class="rounded-circle border">
      </td>
      <td>
